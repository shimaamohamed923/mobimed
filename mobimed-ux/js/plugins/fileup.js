(function ($) {
  var default_options = {
    ...
    templateFile:
      ...
      //   "<span class=\"fileup-edit\" onclick=\"$.fileup('[INPUT_ID]', 'edit', '[FILE_NUM]');\" title=\"[Edit]\"></span>" +
  ...
  var i18n = {
    ru: {
      ...
      error: "Ошибка",
      errorFilesLimit:
        "Количество выбранных файлов превышает лимит (%filesLimit%)",
      errorSizeLimit:
        'Файл "%fileName%" превышает предельный размер (%sizeLimit%)',
      errorFileType: 'Файл "%fileName%" является некорректным',
      errorOldBrowser:
        "Ваш браузер не может загружать файлы. Обновите его до последней версии",
    },
    en: {
      upload: "Upload",
      abort: "Abort",
      ...
      error: "Error",
      errorFilesLimit:
        "The number of selected files exceeds the limit (%filesLimit%)",
      errorSizeLimit: 'File "%fileName%" exceeds the size limit (%sizeLimit%)',
      errorFileType: 'File "%fileName%" is incorrect',
      errorOldBrowser:
        "Your browser can not upload files. Update to the latest version",
    },
  };
    ...
    if (input && input.type === "file") {
      ...
      if (options.files.length > 0) {
        ...
        for (var i = 0; i < options.files.length; i++) {
          ...
          tpl = tpl.replace(
            /\[SIZE_HUMAN\]/g,
            formatHuman(options.files[i].size)
          );
          ...
          $("#" + options.queueID)
            .append(tpl)
            .find(
              ".fileup-progress, .fileup-result, .fileup-upload, .fileup-abort"
            )
            .remove();

          if (options.files[i].downloadUrl) {
            var $name = $(
              "#fileup-" + options.inputID + "-" + input.fileup.nextID
            ).find(".fileup-name");
            if ($name[0]) {
              $name.replaceWith(
                '<a href="' +
                  options.files[i].downloadUrl +
                  '" class="fileup-name" ' +
                  'download="' +
                  options.files[i].name +
                  '">' +
                  options.files[i].name +
                  "</a>"
              );
  ...
  function dropFiles(event) {
    ...
    if (files.length) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (
          options.filesLimit > 0 &&
          Object.keys(files).length >= options.filesLimit
        ) {
  ...
  function appendFiles() {
    if (this.files.length) {
      ...
      for (var i = 0; i < this.files.length; i++) {
        ...
        if (
          options.filesLimit > 0 &&
          Object.keys(this.fileup.files).length >= options.filesLimit
        ) {
  ...
  function uploadFile(input, fileContainer) {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        events.callEvent(input, "success", [
          xhr.responseText,
          file_number,
          file,
        ]);
      } else {
        events.callEvent(input, "error", [
          "bad_status",
          file,
          file_number,
          xhr.responseText,
    ...
    remove: function (inputID, file_number) {
      ...
      if (input && input.type === "file" && typeof input.fileup === "object") {
        ...
        if (file_number === "*") {
          ...
          if (
            events.callEvent(input, "remove", [
              input.fileup.files[file_number],
              total,
              file_number,
            ]) === false
          ) {
          ...
          $("#fileup-" + inputID + "-" + file_number).fadeOut(
            "fast",
            function () {
              $(this).remove();
            }
